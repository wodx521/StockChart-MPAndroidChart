...
public class KDJEntity {
     ...
     */
    public KDJEntity(List<KLineBean> kLineBeens, int n, int m1, int m2) {
        ...
        if (kLineBeens != null && kLineBeens.size() > 0) {
            ...
            float high = (float) kLineBean.high;
            float low = (float) kLineBean.low;

            for (int i = 0; i < kLineBeens.size(); i++) {
                kLineBean = kLineBeens.get(i);
                if (i > 0) {
                    if (n == 0) {
                        high = high > kLineBean.high ? high : (float) kLineBean.high;
                        low = low < kLineBean.low ? low : (float) kLineBean.low;
                    } else {
                        int t = i - n + 1;
                ...
                if (high != low) {
                    rSV = (float) ((kLineBean.close - low) / (high - low) * 100);
                } else {
                    rSV = 0;
                }
